...
"""
═══════════════════════════════════════════════════════════════════
  Lily v9.0 & NullVector v3.0 — Pterodactyl All-in-One Startup
═══════════════════════════════════════════════════════════════════
...
# Bot-to-bot interaction — set each bot's partner ID
# Lily's partner = NullVector's bot ID, NullVector's partner = Lily's bot ID
LILY_BOT_PARTNER_ID   = os.environ.get("LILY_BOT_PARTNER_ID", "")
NV_BOT_PARTNER_ID     = os.environ.get("NV_BOT_PARTNER_ID", "")
...
def create_lily_env(target_dir):
    ...
    content = f"""# Lily v9.0 Configuration — Auto-generated by startup.py
...
# ── Default models (v9.0: cheap models by default!) ──────
DEFAULT_TEXT_MODEL=openai-fast
DEFAULT_IMAGE_MODEL=sana

# ── Bot-to-bot interaction ────────────────────────────────
BOT_PARTNER_ID={LILY_BOT_PARTNER_ID}
...
def create_nv_env(target_dir):
    """Create .env file for NullVector v3.0."""
    env_path = os.path.join(target_dir, ".env")
    if os.path.exists(env_path):
    ...
    content = f"""# NullVector v3.0 Configuration — Auto-generated by startup.py
...
# ── Default models (v3.0: cost-conscious routing) ───────
DEFAULT_TEXT_MODEL=openai-fast
DEFAULT_IMAGE_MODEL=sana
...
# ── Bot-to-bot interaction ────────────────────────────────
BOT_PARTNER_ID={NV_BOT_PARTNER_ID}
"""
    with open(env_path, "w") as f:
        f.write(content)
    log(f"Created {env_path}")
...
def setup_lily():
    """Set up Lily v9.0."""
    log(f"{CYAN}═══ Setting up Lily v9.0 ═══{RESET}")
    ...
    log(f"Lily v9.0 setup complete!")
    return lily_dir

def setup_nullvector():
    """Set up NullVector v3.0."""
    log(f"{CYAN}═══ Setting up NullVector v3.0 ═══{RESET}")
    ...
    log(f"NullVector v3.0 setup complete!")
    return nv_dir
...
def main():
    print()
    print(f"{BOLD}{'═' * 60}")
    print(f"  Lily v9.0 & NullVector v3.0 — Pterodactyl Startup")
    print(f"{'═' * 60}{RESET}")
    print()
    ...
    print()
    if lily_dir:
        log(f"Lily v9.0  →  {lily_dir}")
    if nv_dir:
        log(f"NullVector v3.0  →  {nv_dir}")
    print()
